Задача №4
Таблица содержит данные с логом сообщений в чате клиента с оператором. Все чаты 
инициируются клиентом - первое сообщение всегда клиента, все последующие это 
хаотичный порядок сообщений клиента/оператора. 
Нужно написать SQL-запрос, который для каждого чата вывести скорость ответа оператора 
т.е. через сколько секунд Клиент после своего первого сообщения получил первый ответ 
оператора. В случае, если ответа оператора не последовало проставить «Оператор не 
ответил»
